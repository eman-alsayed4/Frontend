...
const LoginPage = () => {
  ...
  useEffect(() => {
    ...
    window.google.accounts.id.initialize({
      client_id:"560522023117-82fv3equqf93f4a8c2fdro0e7d374gg1.apps.googleusercontent.com",
  ...
  const handleCallbackresponse= (response) => {
    ...
    console.log("handleCallbackresponse", response);
    const userInfoFromToken = jwtDecode(response.credential); //convert token to object
    setLogin(userInfoFromToken);
      toast.success("🦄 LoggedIn Successfully", {
        position: "top-right",
        autoClose: 5000,
    ...
    if (response.status === "select_account") {
      toast.error("Please select an account");
    } else if (response.status === "permission_denied") {
      toast.error("Permission denied");
    } else if (response.status === "signed_out") {
